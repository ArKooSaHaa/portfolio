...
// CardContent structure: { id, title, description, icon, bgClass }
const SlidingCards = ({
  cards,
  className = "",
  cardSize = "w-24 h-24",
  ...
  return (
    ...
    >
          {cards.map(({ id, icon, bgClass = "bg-gradient-to-br from-pink-300 to-orange-200", title, value }, index) => (
        <article
          key={id}
          ...
          <span className={cn("aspect-square grid place-content-center", cardSize)}>
            {icon || (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center p-4 relative text-white">
                {title && <div className="mt-3 text-center"><div className="text-sm font-semibold">{title}</div></div>}
                {value && <div className="text-xs opacity-90 mt-1">{value}</div>}
              </div>
        </article>
      ))}
    </section>
